...
class SoftwarePanel(Screen):
	...
	def checkTraficLight(self):
		...
		try:
			urlopenNFR = "http://dev.nachtfalke.biz/nfr/ampel/index.php"
			d = urlopen(urlopenNFR)
			tmpStatus = d.read()
			if 'ared.png' in tmpStatus:
				self['a_off'].hide()
				self['a_red'].show()
				self['feedstatusRED'].show()
			elif 'ayellow.png' in tmpStatus:
				self['a_off'].hide()
				self['a_yellow'].show()
				self['feedstatusYELLOW'].show()
			elif 'agreen.png' in tmpStatus:
				self['a_off'].hide()
				self['a_green'].show()
