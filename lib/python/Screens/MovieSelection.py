...
class MovieBrowserConfiguration(ConfigListScreen, Screen):
	def __init__(self, session, args = 0):
		...
		configList = [getConfigListEntry(_("Use trash can in movielist"), config.usage.movielist_trashcan, _("When enabled, deleted recordings are moved to the trash can, instead of being deleted immediately.")),
					...
					getConfigListEntry(_("Behavior when a movie is started"), config.usage.on_movie_start, _("Configure the behavior when movie playback is started.")),
					getConfigListEntry(_("Behavior when a movie is stopped"), config.usage.on_movie_stop, _("Configure the behavior when movie playback is manually stopped.")),
					getConfigListEntry(_("Behavior when a movie reaches the end"), config.usage.on_movie_eof, _("Configure the behavior when reaching the end of a movie, during movie playback.")),
					...
					getConfigListEntry(_("Allow quit movieplayer with exit"), config.usage.leave_movieplayer_onExit, _("When enabled, it is possible to leave the movieplayer with exit.")),
					getConfigListEntry(_("Display message before playing next movie"), config.usage.next_movie_msg, _("When enabled, a popup message will be shown when a movie has finished and the next one will start.")),
					getConfigListEntry(_("Behavior of 'pause' when paused"), config.seek.on_pause, _("Configure the behavior of the 'pause' key when movie playback is already paused.")),
